            ...
            <div class="right-search round-search text-left pt-2 mb-0">
                <h3 class="save-action-input" id="interactionName" style="display: none;">
                    ...
                    <div class="report-save">
                        <input type="text" class="form-group reportNameInput" value="{{$reportName}}" name="reportName" autocomplete="off" placeholder="Type File Name to Save" id="reportName" required/> 
...
<div class="modal fade" id="sendMailModalPopup" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="sendmail-modal-title">Send Mail</h4>
      ...
      <div class="modal-body" id="sendmail-modal-body">
        ...
        <form action="{{route('save-pdf',[$userInteractionId,1])}}" id="sendMailForm" class="floating">
            
            <div class="form-group ">    
                <input id="toMail" type="email" placeholder=" " class="form-control" name="toMail" value="" required>
                <label for="toMail" class="float-label">{{ __('To Email') }}</label>
            </div>

            <div class="form-group ">    
                <input id="fromMail" type="text" placeholder=" " class="form-control" name="fromMail" value="{{\Auth::user()->email}}" required readonly="readonly" style="background-color: #ffffff;">
                <label for="fromMail" class="float-label">{{ __('From Email') }}</label>
            </div>
            
            <div class="form-group mb-2">    
                <label for="pdfAttached"><i class="fa fa-file-pdf-o mr-2" aria-hidden="true"></i><em>{{ __('PDF is attached with this email') }}</em></label>
            </div>
      ...
      <div class="modal-footer justify-content-between">
        <button type="button" class="btn btn-default" id="sendMailModalCloseButton" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#sendMailButton").on("click", function(){
            ...
            if($('#sendMailForm')[0].checkValidity()){
                // disable modal popup
                $('#sendMailModalPopup').modal({backdrop: 'static', keyboard: false});
        ...
        deleteAll(); // deletes if any previous interactions data is stored in local storage

        let userStoreddrugsDataIdArray = <?=json_encode($userStoredDrugIds)?>; // gets drugs ids stored for current interaction details
        let userStoredNaturalMedicineDataIdArray = <?=json_encode($userStoredNaturalMedicineIds)?>; // gets natural medicine ids stored for current interaction details
       
        var dataSendArr = { drugsId: userStoreddrugsDataIdArray, medicineId: userStoredNaturalMedicineDataIdArray }; // pass drugs id and natural medicine ids to data json object

        let showInteractionsUrl = "{{route('show-interactions')}}";
        
        // check if drugs id is not empty then show drugs data
        if(userStoreddrugsDataIdArray != null) {
            // get drugs data from the stored data into local storage to get the interactions
            for (const elementDrugId of dataSendArr.drugsId) {
                var storedDrugId = elementDrugId;
                var dataSend1 = { drugsId: storedDrugId};
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if(result.drugsDataId != undefined && result.drugsDataId != ''){
                            if(localStorage.getItem("drugsDataIdEdit") === null) {
                                var drugsDataIdArray = [];
                                drugsDataIdArray.push(result.drugsDataId);
                                localStorage.setItem("drugsDataIdEdit",JSON.stringify(drugsDataIdArray));
                                
                            }else{
                                var oldArr = JSON.parse(localStorage.getItem("drugsDataIdEdit"));
                                oldArr.push(result.drugsDataId);
                                localStorage.setItem("drugsDataIdEdit",JSON.stringify(oldArr));
                            }
                        }
                        ...
                        if(result.naturalMedicinesDataId != undefined && result.naturalMedicinesDataId != ''){
                            if(localStorage.getItem("naturalMedicinesDataIdEdit") === null) {
                                var naturalMedicinesDataIdArray = [];
                                naturalMedicinesDataIdArray.push(result.naturalMedicinesDataId);
                                localStorage.setItem("naturalMedicinesDataIdEdit",JSON.stringify(naturalMedicinesDataIdArray));
                                
                            }else{
                                var oldNaturalArr = JSON.parse(localStorage.getItem("naturalMedicinesDataIdEdit"));
                                oldNaturalArr.push(result.naturalMedicinesDataId);
                                localStorage.setItem("naturalMedicinesDataIdEdit",JSON.stringify(oldNaturalArr));
                            }
                        }
                        ...
                        if(localStorage.getItem("drugs-nm-selected-data-edit") === null) {
                            localStorage.setItem("drugs-nm-selected-data-edit", JSON.stringify(response));
                        }else{
                            existing = JSON.parse(localStorage.getItem("drugs-nm-selected-data-edit"));
                            if(existing){
                                var oldArr = JSON.parse(localStorage.getItem("drugs-nm-selected-data-edit"));
                                oldArr.push(result.section)
                                localStorage.setItem("drugs-nm-selected-data-edit", JSON.stringify(oldArr));
                            }

                        }
                        addDrugsNmData();

                        var getNewDrugsDataId = JSON.parse(localStorage.getItem("drugsDataIdEdit"));
                        var getNewNaturalMedicinesDataId = JSON.parse(localStorage.getItem("naturalMedicinesDataIdEdit"));
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            success: function (result) {
                                localStorage.setItem("interactions-details-edit", JSON.stringify(result));
                                addDrugsNmData();
                            }
                        });
        ...
        // check if natural medicine id is not empty then show drugs data
        if(userStoredNaturalMedicineDataIdArray != null) {
            // get natural medicine data from the stored data into local storage to get the interactions
            for (const elementMedicineId of dataSendArr.medicineId) {
                var storedMedicineId = elementMedicineId;

                var dataSend2 = {medicineId: storedMedicineId };
   ...
   $('input.interactionChecker').typeahead({
      ...
      source: function (query, process) {
        ...
        var drugsDataIds = JSON.parse(localStorage.getItem("drugsDataIdEdit"));
        var naturalMedicinesDataIds = JSON.parse(localStorage.getItem("naturalMedicinesDataIdEdit"));
    ...
    function addDrugsNmData(){
        ...
        if(JSON.parse(localStorage.getItem("drugs-nm-selected-data-edit")) != null){
            var storedNames = JSON.parse(localStorage.getItem("drugs-nm-selected-data-edit"));
            $(".deseases-list").html('');
        ...
        if(JSON.parse(localStorage.getItem("interactions-details-edit")) != null){
            var storedInteractionsNames = JSON.parse(localStorage.getItem("interactions-details-edit"));
    ...
    function deleteDrugs(id){
        ...
        let selectedDrugsNmTags = JSON.parse(localStorage.getItem("drugs-nm-selected-data-edit"));
        ...
        localStorage.setItem('drugs-nm-selected-data-edit', JSON.stringify(selectedDrugsNmTags)); // updates the selected drugs listing
        ...
        let removeDrugsDataIds = JSON.parse(localStorage.getItem("drugsDataIdEdit"));
        ...
        localStorage.setItem('drugsDataIdEdit', JSON.stringify(removeDrugsDataIds));// updates the selected drugs ids listing
    ...
    function deleteNaturalMedicine(id){
        ...
        localStorage.setItem('drugs-nm-selected-data-edit', JSON.stringify(selectedDrugsNmTags)); // updates the selected natural medicine listing
        ...
        var removeNaturalMedicinesIds = JSON.parse(localStorage.getItem("naturalMedicinesDataIdEdit"));
        ...
        localStorage.setItem('naturalMedicinesDataIdEdit', JSON.stringify(removeNaturalMedicinesIds));// updates the selected natural medicine ids listing
    ...
    function deleteAll(){
        ...
        localStorage.removeItem("drugsDataIdEdit"); // remove drug Ids from localStorage
        
        // delete all selected natural medicines ids
        localStorage.removeItem("naturalMedicinesDataIdEdit"); // remove natural medicine Ids from localStorage

        // delete all selected drugs & natural medicines ids
        localStorage.removeItem("drugs-nm-selected-data-edit"); // remove drugs & natural medicine Ids from localStorage
        ...
        localStorage.removeItem("interactions-details-edit"); // remove interaction details from localStorage
    ...
    $( "#saveInteractions" ).click(function(e) {
        ...
        let saveInteractionsUrl = "{{route('update-interaction')}}";
        var csrf_token = $('meta[name="csrf-token"]').attr('content');

        var drugIds = JSON.parse(localStorage.getItem("drugsDataIdEdit"));
        var naturalMedicineIds = JSON.parse(localStorage.getItem("naturalMedicinesDataIdEdit"));
        var reportName = $("#reportName").val();
        var userInteractionReportId = "{{$userInteractionId}}";
        var dataSend = {  _token: csrf_token, userInteractionReportId: userInteractionReportId, drugIds: drugIds, naturalMedicineIds: naturalMedicineIds, reportName: reportName };
        $.ajax({
            ...
            success: function (result) {
                if(result.status == 1){
                    deleteAll();
                    window.location.href = "{{ url('/my-wellkasa-rx') }}"
                }else{
                    window.location.href = "{{ url('/my-wellkasa-rx') }}"
                }                
            },
